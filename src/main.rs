    ...
    fn on_message(&mut self, msg: Message) -> Result<()> {
        if let Message::Text(message_text) = msg {
            ...
            if message["op"] == 0 {
                ...
                if let Value::String(ref requester) = message["d"]["requester"]["displayName"] {
                    println!(
                        "[1] Currently playing: {} - {} [Requested By: {}]",
                        message["d"]["song"]["title"].as_str().unwrap().to_string(),
                        message["d"]["song"]["artists"][0]["name"]
                            .as_str()
                            .unwrap()
                            .to_string(),
                        requester
                ...
                if checkArgs("-s", "--stats"){
                    print!(
                        "Listeners: {}",
                        message["d"]["listeners"]
                    );
                    if let Value::String(ref event) = message["d"]["event"] {
                        print!(". Event: {}", event);
                    }
                    print!("\r\n");
                }
...
fn main() {
    ...
    let update_reel = thread::spawn(move || {
        connect("wss://listen.moe/gateway_v2", |out| UpdateHandler {
            sender: out,
        })
        .unwrap()
    });
